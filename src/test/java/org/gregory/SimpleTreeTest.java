...
class SimpleTreeTest {
    ...
    @Test
    void evenTree_4Nodes_NoEdgeForDelete() {
        SimpleTree<Integer> treeActual = new SimpleTree<>(new SimpleTreeNode<Integer>(1));
        treeActual.addChildArray(treeActual.getRoot(), new SimpleTreeNode[] {
                new SimpleTreeNode(2),
                new SimpleTreeNode(3),
                new SimpleTreeNode(6),
        });

        treeActual.addChildArray(treeActual.getRoot().getChildren().get(0), new SimpleTreeNode[] {
                new SimpleTreeNode(5),
                new SimpleTreeNode(7)
        });
        treeActual.AddChild(treeActual.getRoot().getChildren().get(1), new SimpleTreeNode<>(4));

        treeActual.AddChild(treeActual.getRoot().getChildren().get(2), new SimpleTreeNode<>(8));
        treeActual.addChildArray(treeActual.getRoot().getChildren().get(2).getChildren().get(0), new SimpleTreeNode[] {
                new SimpleTreeNode(9),
                new SimpleTreeNode(10)
        });

        assertThat(treeActual.EvenTrees(), is(List.of(1,3,1,6)));
    }
}